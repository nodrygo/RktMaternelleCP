...
(define (curchar)
    (string-ref currentword  currentpos))
...
(define (newword)
  ...
  (send canvas  refresh-now)
    (play-sound (format "sons/~a.wav" currentword) #f)
    (play-sound (format "sons/~a.wav" (curchar)) #f)
 ; )    
)
           ...
           (if (eq? k (string-ref currentword  currentpos))
               (begin 
                      ...
                      (when  (< currentpos (string-length currentword))
                        (if (char-whitespace? k)
                          (play-sound (format "sons/espace.wav"  (curchar)) #f)   
                          (play-sound (format "sons/~a.wav"  (curchar)) #f))
                      )) 
               (begin
                      (set! err #t)
